			    ...
			    struct platform_device *pdev)
{
	...
	for_each_child_of_node(node, child_node) {
		...
		printk(KERN_INFO "---> KEY: key code = %d, key label = %s\n", pdata->button[i].code, pdata->button[i].desc);
		pdata->button[i].type = rk_key_type_get(child_node, &pdata->button[i]);
		switch (pdata->button[i].type) {
		case TYPE_GPIO:
			printk(KERN_INFO "---> KEY: key type is GPIO\n");
		...
		case TYPE_ADC:
			printk(KERN_INFO "---> KEY: key type is ADC\n");
		...
		default:
			printk(KERN_INFO "---> KEY: key type is default\n");
