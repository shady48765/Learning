...
#if 0	
#ifndef BUILD_LK
	printk("[Kernel/LCM] [it6151_read_byte] dev_addr = 0x%x, write_data[0x%x] = 0x%x \n", dev_addr, write_data[0], write_data[1]);
#else
	printf("[LK/LCM] [it6151_read_byte] dev_addr = 0x%x, write_data[0x%x] = 0x%x \n", dev_addr, write_data[0], write_data[1]);
#endif
#endif
...
//===================================================================	
	...
	elink_lcm_avdd(1);//LCM 3.3V GPIO63
	MDELAY(10);

	// elink_lcm_dvdd(1); //EDP 1.8V VIO18_PMU		
	elink_lcm_edp_power(1);//EDP 1.2V GPIO128
	MDELAY(10);
	
	elink_lcm_standby(0);//edp IT6151 GPIO147 Standby LOW Activity
	// elink_lcm_edp_standby(0);
	MDELAY(10);	
	
	elink_lcm_edp_rst(1);//EDP RESET GPIO96
	MDELAY(10);	
}
