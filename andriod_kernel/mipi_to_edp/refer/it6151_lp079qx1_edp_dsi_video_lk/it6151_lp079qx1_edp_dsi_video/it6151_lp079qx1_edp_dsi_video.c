...
#define GPIO_LCD_PWE_EN        (GPIO63  | 0x80000000)
#define GPIO_EDP_PWE12_EN      (GPIO128 | 0x80000000)
#define GPIO_EDP_RST_EN        (GPIO96  | 0x80000000)
#define GPIO_LCD_BL_EN         (GPIO8   | 0x80000000)

// #define GPIO_EXT_POWER_EN      (GPIO92  | 0x80000000)
...
{
	printf("[LK/LCM] %s(%d), LK \n", __func__, enabled?1:0);

    if (enabled)
    {
		mt6328_set_register_value(PMIC_RG_VRF18_1_ON_CTRL,0x3);	
...
static kal_uint32 it6151_i2c_write_byte(u8 dev_addr,u8 addr, u8 data)
{
...
#if 1	
#ifndef BUILD_LK
	printk("[Kernel/LCM] [it6151_i2c_write_byte] dev_addr = 0x%x, write_data[0x%x] = 0x%x \n", dev_addr, write_data[0], write_data[1]);
#else
	printf("[LK/LCM] [it6151_i2c_write_byte] dev_addr = 0x%x, write_data[0x%x] = 0x%x \n", dev_addr, write_data[0], write_data[1]);
#endif
#endif
...
static kal_uint32 it6151_i2c_read_byte(u8 dev_addr,u8 addr, u8 *dataBuffer)
{
  	kal_uint32 ret_code = I2C_OK;
	u8 len;
	struct mt_i2c_t i2c;
...
#if 1	
#ifndef BUILD_LK
	printk("[Kernel/LCM] [it6151_i2c_read_byte] dev_addr = 0x%x, read_data[0x%x] = 0x%x \n", dev_addr, addr, *dataBuffer);
#else
	printf("[LK/LCM] [it6151_i2c_read_byte] dev_addr = 0x%x, read_data[0x%x] = 0x%x \n", dev_addr, addr, *dataBuffer);
#endif
#endif
...
#else
	printf("[LK/LCM] IT6151_dump_register end !!!\n");
#endif
}
...
static unsigned int IT6151_ESD_Check(void)
{
	...
	#ifndef BUILD_LK
		printk("[LCM] Current DevID=%02X%02X\n", DevID[1], DevID[0]);
		printk("[LCM] Current VenID=%02X%02X\n", VenID[1], VenID[0]);
			#endif
...
{	
	...
	//params->dsi.ufoe_vlc_disable = 1;
	
	params->dsi.vertical_sync_active				= 6;	// 5; // 1;	//	5;	//	2;	//	4;
	params->dsi.vertical_backporch					= 6; // 4;	// 3;	//	4;	//	1;	//	8;
	params->dsi.vertical_frontporch 				= 30;  // 16; //30;	//16;	//	36; //	2;	//	8;
	params->dsi.vertical_active_line				= FRAME_HEIGHT; 

	params->dsi.horizontal_sync_active				= 32; // 30; //32;	//16;	//	44; //	8;	//	16;
	params->dsi.horizontal_backporch				= 103; // 138; //103;	//12;	//	88; //	8;	//	48;
	params->dsi.horizontal_frontporch				= 85; // 60; //88;	//36;	//	148;	//	112;	//	12;
	params->dsi.horizontal_active_pixel 			= FRAME_WIDTH;

  	params->dsi.PLL_CLOCK = 440; // 299 ;//240	//319;ngel][PKG] LOCAL_PATH=vendor/mediatek/proprietary/packages/apps/Bluetooth/build
	params->dsi.cont_clock = 1;
	params->dsi.ssc_disable = 1;
	params->dsi.HS_TRAIL = 0x8;
	params->dsi.HS_ZERO = 0xF;
...
static void lcm_power_init(void)
{
	...
	// elink_lcm_avdd(0);
	// elink_lcm_dvdd(0);
	// elink_lcm_edp_power(0);
	// elink_lcm_ext_power(0);
	// elink_lcm_reset(0);
	// lcm_set_gpio_output(GPIO_EXT_POWER_EN, GPIO_OUT_ZERO);//GPIO92 12V for LCM BL AK22
	lcm_set_gpio_output(GPIO_LCD_BL_EN, GPIO_OUT_ZERO);//BL EN GPIO8	
	lcm_set_gpio_output(GPIO_EDP_RST_EN, GPIO_OUT_ZERO);//EDP RST GPIO96	
	elink_lcm_standby(1);
	// elink_lcm_edp_standby(1);
	lcm_set_gpio_output(GPIO_LCD_PWE_EN, GPIO_OUT_ZERO);//LCM 3.3V EN GPIO63
	MDELAY(10);
	
//===================================================================	
	// lcm_set_gpio_output(GPIO_EXT_POWER_EN, GPIO_OUT_ONE);//GPIO92 12V for LCM BL AK22
	
	// elink_lcm_avdd(1);
	lcm_set_gpio_output(GPIO_LCD_PWE_EN, GPIO_OUT_ONE);//LCM 3.3V EN GPIO63
	MDELAY(10);	
	// elink_lcm_dvdd(0); //EDP 1.8V VIO18_PMU 
	lcm_set_gpio_output(GPIO_EDP_PWE12_EN, GPIO_OUT_ONE);//EDP 1.2V EN GPIO128
	MDELAY(10);	
	// elink_lcm_ext_power(1);
	// elink_lcm_edp_power(1);
	// MDELAY(10);	
	
	elink_lcm_standby(0);//GPIO147 edp IT6151 Standby LOW Activity
	// elink_lcm_edp_standby(0);
	MDELAY(10);	

	lcm_set_gpio_output(GPIO_EDP_RST_EN, GPIO_OUT_ONE);//EDP RST GPIO96	
	// elink_lcm_reset(1);
	MDELAY(10);	
}
...
{			
	...
	//open BL in platform.c   in lk_mt65xx_backlight_on();
	// lcm_set_gpio_output(GPIO_LCD_BL_EN, GPIO_OUT_ONE);//BL EN GPIO8		
}
