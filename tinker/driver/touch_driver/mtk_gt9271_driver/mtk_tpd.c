...
static int tpd_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	switch (blank) {
	case FB_BLANK_UNBLANK:				// screen status ---> file fb.h
...
static int tpd_probe(struct platform_device *pdev)
{
	...
	TPD_DMESG("enter %s, %d\n", __func__, __LINE__);
	// misc_device register, touch panel device regist as misc device
	/**
	 * 分配设备节点 创建设备，platform 注册 tpd_misc_device (注册 touch 设备) 到 misc device  添加 class,
	 */
	if (misc_register(&tpd_misc_device))
		pr_err("mtk_tpd: tpd_misc_device register failed\n");
	// find platform_device gpio and configuration parameter
	tpd_get_gpio_info(pdev);
	// struct tpd_device * tpd same gloable var with gslx680.c
	 ...
	 * allocate parameter for new input device 分配 一个输入设备 给 tpd->dev
	 * 函数在内存中为输入设备结构体分配一个空间,并对其主要的成员进行了初始化.
	 * 在下面的参数设置完成后调用 input_register_device(tpd->dev) 注册到输入子系统中
	 *
	 */
...
// modified by elink_phil start <<<
	elink_tp_init(elink_tp_list, sizeof(elink_tp_list) / sizeof(ELINK_TP_INFO), &TPD_RES_X, &TPD_RES_Y);
...
// modified end >>>
	...
	if (1600 == TPD_RES_Y)
		TPD_RES_Y = 1536;
	printk("mtk_tpd: TPD_RES_X = %lu, TPD_RES_Y = %lu\n", TPD_RES_X, TPD_RES_Y);
	...
	set_bit(ABS_PRESSURE, tpd->dev->absbit);		// mulit point support
	...
	// tpd_probe param pdev->dev transmit to gloable value tpd->dev
	tpd->tpd_dev = &pdev->dev;
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		/* add tpd driver into list */
		if (tpd_driver_list[i].tpd_device_name != NULL) {		// if have new devices
			// call specific device tpd_local_init to finished touch panel chip device and i2c register
			tpd_driver_list[i].tpd_local_init();
			/* msleep(1); */
			// registe one success break for loop
	...
	// if filed up g_tpd_drv struct (static struct tpd_driver_t *g_tpd_drv;)
	if (g_tpd_drv == NULL) {
	...
	// register nodtifier
	// for get LCD status(light or dim) though framebuffer
	...
	// register touch panel as input device
...
int tpd_driver_add(struct tpd_driver_t *tpd_drv)
{
	...
	/* R-touch */
	if (strcmp(tpd_drv->tpd_device_name, "generic") == 0) {					//resistor screen
	...
	// others art capacity screen
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		/* add tpd driver into list */
		if (tpd_driver_list[i].tpd_device_name == NULL) {						// find a empty devices name
			tpd_driver_list[i].tpd_device_name = tpd_drv->tpd_device_name;
			tpd_driver_list[i].tpd_local_init = tpd_drv->tpd_local_init;		// call tpd_local_init function
		...
		if (strcmp(tpd_driver_list[i].tpd_device_name, tpd_drv->tpd_device_name) == 0)		// driver is exit or not
