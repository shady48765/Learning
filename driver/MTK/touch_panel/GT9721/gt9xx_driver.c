...
#ifdef CONFIG_OF
static const struct of_device_id tpd_of_match[] = { {.compatible = "mediatek,silead_touch"}, {},};
#endif
...
static int of_get_gt9xx_platform_data(struct device *dev)
{
	...
	if (dev->of_node) {
		...
		GTP_INFO("g_vproc_en_gpio_number %d\n", tpd_rst_gpio);
		GTP_INFO("g_vproc_vsel_gpio_number %d\n", tpd_int_gpio);
	}
#endif
	return 0;
}
...
{
#if 0	
	gtp_gpio_output(GTP_IRQ_GPIO, 1);//0
	msleep(ms);
...
// tpd_gt9xx_common.h 中没有定义 / macro has not defined in file tpd_gt9xx_common.h
...
// macro has not defined in file tpd_gt9xx_common.h
#ifdef CONFIG_GTP_HAVE_TOUCH_KEY
