...
// modified by elink_phil start <<<
...
#endif
	printk(KERN_WARNING "--------> [get int] (%s)\n", __func__);
	node1 = of_find_matching_node(node1, touch_of_match);
	printk(KERN_WARNING "--------> [of_find_matching_node] (%s)\n", __func__);
	if (node1) {
	#if 0
	...
	#endif
// modified end >>>
		printk(KERN_WARNING "--------> [of_find_matching_node success] (%s)\n", __func__);
		...
		printk(KERN_WARNING "--------> [of_find_matching_node failed] (%s)\n", __func__);
	...
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		...
		if (tpd_driver_list[i].tpd_device_name != NULL) {
			...
			if (tpd_load_status == 1) {
                ...
                printk(KERN_WARNING "[mtk-tpd] [can't find driver] (%s)\n", __func__);
		}
	}
	if (g_tpd_drv == NULL) {
...
int tpd_driver_add(struct tpd_driver_t *tpd_drv)
{
	...
	/* check parameter */
	if (tpd_drv == NULL){
        printk(KERN_WARNING "--------> [tpd_drv is NULL] (%s)\n", __func__);
		return -1;
    }
	...
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		/* add tpd driver into list */
		if (tpd_driver_list[i].tpd_device_name == NULL) {
            printk(KERN_WARNING "--------> [add device %s] (%s)\n", tpd_drv->tpd_device_name, __func__);
