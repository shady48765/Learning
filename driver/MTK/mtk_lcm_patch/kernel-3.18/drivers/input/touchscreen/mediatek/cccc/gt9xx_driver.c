...
#include "tpd.h"
 #include "./include/config/gt9xx_config.h"
...
static const struct of_device_id gt9xx_dt_match[] = { {.compatible = "mediatek,touch"}, {},
};
...
static int of_get_gt9xx_platform_data(struct device *dev)
{
	...
	if (dev->of_node) {
		...
		match = of_match_device(of_match_ptr(.), dev);
		if (!match) {
	...
	if (node) {
		...
		if (!int_type) {/* EINTF_TRIGGER */
			...
			if (ret > 0)
				GTP_ERROR("tpd request_irq IRQ LINE NOT AVAILABLE!.");
#else
			ret = equest_irq(touch_irq, tpd_interrupt_handler, IRQF_TRIGGER_FALLING, TPD_DEVICE, NULL);
			if (ret > 0)
				GTP_ERROR("tpd request_irq IRQ LINE NOT AVAILABLE!.");
        }
	} else {
